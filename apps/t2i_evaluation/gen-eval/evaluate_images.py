...
def parse_args():
    ...
    parser.add_argument("--result-csv-path", type=str, default="path of result csv file")
    parser.add_argument("--eval-metadata-dir", type=str, default="path of evaluation metadata json")
...
def read_result_csv(csv_path):
    ...
    image_dict = {} # {prompt: [image1, image2, image3], ...}
...
def process_json_file(json_path):
    ...
    with open(json_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            prompt_key = data['prompt']
            result_dict[str(prompt_key)] = data

    return result_dict

def main(args):

    # {prompt_filename: [image1, image2, image3], ...}
    image_dict = read_result_csv(args.result_csv_path)
    metadata_dict = process_json_file(args.eval_metadata_dir)

    full_results = []

    for prompt, paths in tqdm(image_dict.items()):
        metadata = metadata_dict[str(prompt)]
        for path in paths:
            result = evaluate_image(path, metadata)
