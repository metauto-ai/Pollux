...
from apps.Simple_DiT.transformer import (
    DiTransformerArgs,
    DiTransformer,
...
class TrainArgs:
    ...
    model: DiTransformerArgs = field(default_factory=DiTransformerArgs)
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        diffusion_scheduler = RectFlow(args.scheduler)
        ...
        while train_state.step < args.steps:
            ...
            noised_x, t, target = diffusion_scheduler.sample_noised_input(image)
            data_load_time = round(timer() - data_load_start, 4)
            nwords_since_last_log += image.numel()
            ...
            output = model(x=noised_x,time_steps=t,context=label)
