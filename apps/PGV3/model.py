...
from apps.PGV3.modules.transformer import ModelArgs as PolluxGenArgs
from apps.PGV3.modules.transformer import PGV3
...
class Latent_Pollux(nn.Module):
    ...
    def forward(self, batch: dict[str:any]) -> dict[str:any]:
        if hasattr(self, "compressor"):
            batch["plan_latent_code"] = self.compressor.encode(batch["image"])
        out_put, loss = self.gen_model(batch)
        return out_put, loss
    ...
    def init_weights(self, args: ModelArgs):
        if args.pre_trained_weight:
            args.gen.llm.pre_trained_path = None
...
def build_fsdp_grouping_plan(model_args: ModelArgs):
    ...
    for i in range(model_args.gen.llm.n_layers):
        group_plan.append((f"gen_model.llm.layers.{i}", False))
    group_plan.append(("gen_model.latent_head", True))
