...
class LatentGenerator(nn.Module):
    def __init__(self,cfg:GeneratorArgs, model:nn.Module):
        ...
        self.vae_scale_factor = (
            2 ** (len(self.model.compressor.vae.config.block_out_channels) - 1) if hasattr(self, "vae") and self.vae is not None else 8
        )
    ...
    def prepare_negative_context(self,context):
        return torch.tensor([self.model.transformer.num_classes]*context.size(0)).to(context.device)
    ...
    def forward(self,context:torch.Tensor)->torch.Tensor:
        ...
        sigmas = np.linspace(1.0, 1 / self.num_inference_steps, self.num_inference_steps) if self.sigma is None else self.sigma
        timesteps, _ = retrieve_timesteps(self.scheduler, self.num_inference_steps, context.device, sigmas=sigmas,mu=mu)
        ...
        for i, t in enumerate(timesteps):
            ...
            noise_pred = self.model.transformer(x=latent_model_input,time_steps=timestep,context=context)
            noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
            noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
            latent = self.scheduler.step(noise_pred, t, latent, return_dict=False)[0]
        
        
        latent = (latent / self.model.compressor.vae.config.scaling_factor) + self.model.compressor.vae.config.shift_factor
...
def main():
    ...
    model,_ = load_consolidated_model(cfg.ckpt_dir,model_cls=LatentTransformer,model_args_cls=ModelArgs)
