...
from kernels.configure import calculate_settings
import math
...
@triton.jit
def rmsn_fwd_kernel(
    inp_ptr, out_ptr, scale_ptr, inp_row_stride, out_row_stride, g_row_stride,
    ...
    sum_sqr_x = tl.sum(x * x, axis=0)
    rms = tl.sqrt(sum_sqr_x / inp_row_stride + 1e-6)
...
@triton.jit
def rmsn_bwd_kernel(
    grad_out_ptr, inp_ptr, scale_ptr, grad_input_ptr, grad_scale_ptr, inp_row_stride,
    BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(0)
    cols = tl.arange(0, BLOCK_SIZE)
    ...
    if pid == 0:
        grad_scale = tl.sum(grad_out * x_norm, axis=0)
...
class RMSNorm(torch.autograd.Function):
    ...
    def forward(ctx, x, scale=None):
        ...
        if scale is None:
            scale = torch.ones(cols, device='cuda')
