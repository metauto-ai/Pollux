...
from kernels.rope1d import RoPE1D_triton, get_cos_sin
from kernels.rmsnorm import RMSNorm as RMSNormFunction
from kernels.cross_entropy import fast_cross_entropy
...
class FFN_SwiGLU(nn.Module):
    ...
    def forward(self, x):
        var1 = self.w1(x)
        var2 = self.v(x)
        swish = SiLU.apply(var1 * self.beta)
        gate_out = swish * var2
        x = self.w2(gate_out)
        return x
...
class SelfAttn(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.wo.flag = 1
        # Remove vanilla Rotary and store cos/sin buffers
    ...
    def forward(self, x):
        ...
        # Use Triton RoPE kernel
        if T != self.seq_len_cached:
            self.cos_cached, self.sin_cached = get_cos_sin(self.h, T)
            self.seq_len_cached = T
            
        q = RoPE1D_triton.apply(q, self.cos_cached, self.sin_cached, self.h)
        k = RoPE1D_triton.apply(k, self.cos_cached, self.sin_cached, self.h)
...
class Gpt(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            loss = fast_cross_entropy(x.view(-1, self.config.vocab_size), targets.view(-1)) #use our custom cross entropy
            return x, loss
        else:
            return x
...
def train(model, ctx, opt, config, epochs, grad_accum_steps=16):
    ...
    scaler = torch.amp.GradScaler(enabled=(torch.cuda.is_available() and config.dtype == 'float16'))
    x, y = get_batch('train')
